...
using SLSD = struct __attribute__((__packed__)) _LICH {
	...
	uint8_t  metadata[14]; //bytes for the metadata
}; // 6 + 6 + 2 + 14 = 28 bytes = 224 bits

// the one and only frame
using SSMFrame = struct __attribute__((__packed__)) _sm_frame {
	uint8_t  magic[4];
	uint16_t streamid;
	SLSD lsd;
	...
	uint16_t GetFrameType  (void) const        { return ntohs(lsd.frametype); }
	...
	void     SetFrameType  (const uint16_t ft) { lsd.frametype = htons(ft);   }
	void     SetFrameNumber(const uint16_t fn) { framenumber    = htons(fn);   }

}; // 4 + 2 + 28 + 2 + 16 + 2 = 54 bytes = 432 bits

using SPMFrame = struct __attribute__((__packed__)) _pm_frame {
	uint8_t  magic[4];
	SLSD     lsd;
	uint16_t crc;
	uint8_t  data[825];
